...
export const setDefaultValue = <T extends Object>(obj:T, defaultValue:any = void 0) => new Proxy(obj,{
    get:(target:T,prop: Extract<keyof T, string> ) => target[prop] ?? defaultValue
})

export const getLastItem = (arr:Array<any>) => new Proxy(arr,{
    get:(target,prop:string,self) => Reflect.get(target, +prop < 0 ? String(target.length + +prop): prop, self)
})

export const hidePrivate = <T extends Object>(obj:T, prefix="_") => new Proxy(obj,{
...
export const getCookieObject = ():any => {
    const cookies = document.cookie
        ...
        .reduce(
            (cks, ck) => ({
                [ck.substring(0, ck.indexOf("=")).trim()]: ck.substring(
                    ck.indexOf("=") + 1
                ),
                ...cks
            }),
...
export const proxyApply = <T extends Function>(fn:T) => new Proxy(fn,{
